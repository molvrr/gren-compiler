      ...
      0x2D {- - -} ->
        let !pos1 = plusPtr pos 1
         in if pos1 < end && P.unsafeIndex pos1 == 0x2D {- - -}
              then eatLineComment (plusPtr pos 2) end row (col + 2)
              else (# Good, pos, row, col #)
      0x0D {- \r -} ->
  ...
  if pos >= end
    then (# Good, pos, row, col #)
      ...
      let !word = P.unsafeIndex pos
       in if word == 0x0A {- \n -}
            then eatSpaces (plusPtr pos 1) end (row + 1) 1
            else
...
eatMultiCommentHelp pos end row col openComments =
  if pos >= end
    then (# MultiEndless, pos, row, col #)
      ...
      let !word = P.unsafeIndex pos
       in if word == 0x0A {- \n -}
            then eatMultiCommentHelp (plusPtr pos 1) end (row + 1) 1 openComments
            else
              if word == 0x09 {- \t -}
                then (# MultiTab, pos, row, col #)
                  ...
                  if word == 0x2D {- - -} && P.isWord (plusPtr pos 1) end 0x7D {- } -}
                    then
                      if openComments == 1
                        then (# MultiGood, plusPtr pos 2, row, col + 2 #)
                        else eatMultiCommentHelp (plusPtr pos 2) end row (col + 2) (openComments - 1)
                    else
                      if word == 0x7B {- { -} && P.isWord (plusPtr pos 1) end 0x2D {- - -}
                        then eatMultiCommentHelp (plusPtr pos 2) end row (col + 2) (openComments + 1)
                        else
                          let !newPos = plusPtr pos (P.getCharWidth word)
